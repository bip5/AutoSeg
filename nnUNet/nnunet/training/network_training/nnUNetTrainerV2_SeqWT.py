...
class nnUNetTrainerV2_SeqWT(nnUNetTrainerV2):
    ...
    def get_basic_generators(self):
        """Override to use our custom batch size and patch_size for both."""
        self.load_dataset()
        self.do_split()

        if self.threeD:
            dl_tr = DataLoader3D(self.dataset_tr, self.patch_size, self.patch_size,
                                 self.seqwt_batch_size, False,
                                 oversample_foreground_percent=self.oversample_foreground_percent,
                                 pad_mode="constant", pad_sides=self.pad_all_sides)
            dl_val = DataLoader3D(self.dataset_val, self.patch_size, self.patch_size,
                                  self.seqwt_batch_size, False,
                                  oversample_foreground_percent=self.oversample_foreground_percent,
                                  pad_mode="constant", pad_sides=self.pad_all_sides)
        else:
    ...
    def initialize_network(self):
        ...
        # CPU only — do NOT call network.cuda()
