...
class DualInputGenericTransform(AbstractTransform):
    """
    Applies a base transform to get a 'clean' anchor, then applies a noise transform pipeline
    to generate the 'augmented' input.
    Resulting data dict will have:
    - 'data': The augmented data (result of base + noise)
    - 'data_clean': The clean anchor (result of base only)
    - 'target': The target segmentation (transformed by base + noise if applicable)
    """
    def __init__(self, base_transform, noise_transform):
        self.base_transform = base_transform
        self.noise_transform = noise_transform

    def __call__(self, **data_dict):
        # 1. Apply Base Transform (e.g. spatial crop/scale)
        # This provides the aligned geometric basis for both clean and augmented views
        data_dict = self.base_transform(**data_dict)
        
        # 2. Store the 'clean' aligned data
        # We assume 'data' key holds the image. 
        # We Copy it because the next transform might modify it in-place.
        if 'data' in data_dict:
            data_dict['data_clean'] = np.copy(data_dict['data'])
        
        # 3. Apply Noise Transform (e.g. rotation, deformation, intensity noise)
        # This modifies 'data' and potentially 'target' (if noise includes spatial transforms)
        data_dict = self.noise_transform(**data_dict)
