...
def merge(args):
    ...
    if not isfile(out_file):
        res1 = np.load(file1)['softmax'].astype(np.float32)
        res2 = np.load(file2)['softmax'].astype(np.float32)
        ...
        # The npz softmax is already at original_size_of_raw_data (not size_after_cropping)
        # So we can directly argmax and save without resampling
        seg = mn.argmax(0).astype(np.uint8)
        
        # Create nifti with correct geometry
        seg_itk = sitk.GetImageFromArray(seg)
        seg_itk.SetSpacing(props['itk_spacing'])
        seg_itk.SetOrigin(props['itk_origin'])
        seg_itk.SetDirection(props['itk_direction'])
